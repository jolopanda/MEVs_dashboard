...
const App: React.FC = () => {
  ...
  // Check if we are in "embed mode" via URL parameter
  ...
  return (
    <div className={`min-h-screen ${isEmbed ? 'pb-4 bg-white' : 'pb-12 bg-[#f8fafc]'}`}>
      ...
      <main className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${isEmbed ? 'mt-4' : 'mt-8'}`}>
        ...
        {state.loading ? (
          ...
          <>
            <div className={`grid grid-cols-1 ${isEmbed ? 'md:grid-cols-1 lg:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'} gap-6`}>
              {state.indicators.map(indicator => (
                <IndicatorChart key={indicator.id} indicator={indicator} />
              ))}
            </div>

            {/* Official Primary Sources Section - Simplified in Embed Mode */}
            {!isEmbed && (
            ...
            {/* Grounding Sources Listing - Hidden in Embed Mode to save space */}
            {!isEmbed && (
